...
function SetFont($family, $style='', $size=0)
{
    ...
    if(!isset($this->fonts[$fontkey]))
    {
        ...
        if(in_array($family,$this->CoreFonts))
        {
            ...
            if(!isset($this->fonts[$fontkey]))
            {
                // Load core font metrics directly
                $this->fonts[$fontkey] = $this->_getCoreFontMetrics($family, $style);
            }
        }
        else
...
protected function _getCoreFontMetrics($family, $style)
{
    $name = ucfirst($family);
    if($style=='B')
        $name .= '-Bold';
...
protected function _beginpage($orientation, $size, $rotation)
{
    ...
    if($orientation!=$this->DefOrientation || $size[0]!=$this->DefPageSize[0] || $size[1]!=$this->DefPageSize[1])
        $this->PageInfo[$this->page] = array($orientation, $size);
    if($rotation!=0)
    {
        if($rotation%90!=0)
...
protected function _tounicodecmap($uv)
{
    ...
    $this->_put('/CIDInit /ProcSet findresource begin');
    $this->_put('12 dict begin');
    $this->_put('begincmap');
    $this->_put('/CIDSystemInfo');
    $this->_put('<</Registry (Adobe)');
    $this->_put('/Ordering (UCS)');
    $this->_put('/Supplement 0');
    $this->_put('>> def');
    $this->_put('/CMapName /Adobe-Identity-UCS def');
    $this->_put('/CMapType 2 def');
    $this->_put('1 begincodespacerange');
    $this->_put('<00> <FF>');
    $this->_put('endcodespacerange');
    if($nbr>0)
    {
        $this->_put($nbr.' beginbfrange');
        $this->_put($ranges);
        $this->_put('endbfrange');
    }
    if($chars!='')
    {
        $this->_put($nbr.' beginbfchar');
        $this->_put($chars);
        $this->_put('endbfchar');
    }
    $this->_put('endcmap');
    $this->_put('CMapName currentdict /CMap defineresource pop');
    $this->_put('end');
    $this->_put('end');
}
...
protected function _putcatalog()
{
    ...
    if($this->ZoomMode!='default')
        $this->_put('/ViewerPreferences <</HideToolbar false /HideMenubar false /HideWindowUI false>>');
    $this->_put('/OpenAction ['.$n.' 0 R /'.$this->ZoomMode.' null]');
